...
export const subscribePendingKitchenPrintJobs=(storeId:string,onChange:(jobs:KitchenPrintJob[])=>void,onError?:(error:Error)=>void)=>subscribeRealtimeResource({storeId,events:["kitchen-jobs-updated"],loader:()=>load(storeId),onChange,onError});
