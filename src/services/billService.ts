...
export async function getBills(options?: { startDate?: Date; endDate?: Date; limitCount?: number; storeId?: string; includeCancelled?: boolean; status?: "active" | "cancelled"; shiftId?: string; shiftIds?: string[] }) {
  return (await posRequest<{ items: Bill[] }>("bills", {}, { storeId: options?.storeId || "cafe", startDate: options?.startDate, endDate: options?.endDate, limit: options?.limitCount || 100, includeCancelled: options?.includeCancelled === true, status: options?.status, shiftId: options?.shiftId, shiftIds: options?.shiftIds?.join(",") })).items;
}
