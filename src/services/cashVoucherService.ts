...
export type CashVoucher = NewCashVoucher & { id: string; code: string; createdAt?: ApiTimestamp; happenedAt?: ApiTimestamp };
...
export async function getCashVouchers(options: { storeId: string; startDate?: Date; endDate?: Date; limitCount?: number; shiftId?: string }) { return (await posRequest<{ items: CashVoucher[] }>("vouchers", {}, { storeId: options.storeId, startDate: options.startDate, endDate: options.endDate, limit: options.limitCount || 500, shiftId: options.shiftId })).items; }
