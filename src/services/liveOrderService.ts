...
const loadOrders = async (storeId: string) => (await posRequest<{ items: LiveOrder[] }>("live-orders", {}, { storeId })).items;
export const subscribeLiveOrders = (storeId: string, onChange: (orders: LiveOrder[]) => void, onError?: (error: Error) => void) => subscribeByPolling(() => loadOrders(storeId), onChange, onError);
