...
$sourcePath = $argv[1] ?? (__DIR__ . '/../artifacts/tn-company-admin.apk');
if (!is_file($sourcePath)) {
    fwrite(STDERR, "APK not found: {$sourcePath}\n");
    exit(1);
}

function admin_mobile_apk_config(string $configKey, string $environmentKey): string
{
    global $config;
...
$accountId = admin_mobile_apk_config('r2_account_id', 'R2_ACCOUNT_ID');
$endpoint = admin_mobile_apk_config('r2_endpoint', 'R2_ENDPOINT');
...
$storage = new R2Storage(
    $endpoint,
    admin_mobile_apk_config('r2_bucket', 'R2_BUCKET'),
    admin_mobile_apk_config('r2_access_key_id', 'R2_ACCESS_KEY_ID'),
    admin_mobile_apk_config('r2_secret_access_key', 'R2_SECRET_ACCESS_KEY')
);

$key = 'mobile/tn-company-admin.apk';
...
if ($curl === false) throw new RuntimeException('Cannot initialize APK verification request.');
