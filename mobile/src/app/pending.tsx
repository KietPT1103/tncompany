...
export default function PendingScreen() {
  ...
  return <Screen><Pressable onPress={() => router.back()}><Text style={styles.back}>‹ Quay lại</Text></Pressable>
    ...
    <FlatList data={items} keyExtractor={(item) => item.id} contentContainerStyle={styles.list}
      ...
      renderItem={({ item }) => <Pressable style={styles.card} onPress={() => router.push({ pathname: "/receipt/[id]", params: { id: item.id } })}>
...
const styles = StyleSheet.create({
  ...
  meta: { color: "#64748b", marginTop: 5 }, action: { color: "#059669", fontWeight: "800" }
});
